...
public interface LogRepository extends PagingAndSortingRepository<LogEntity, Long> {
    ...
    @Query(value = "SELECT l.message as message, l.dateTime as dateTime FROM LogEntity l LEFT JOIN OwnerEntity o ON l.owner.id = o.id LEFT JOIN User u ON u.id = o.user.id WHERE u.username = :username ORDER BY l.id DESC")
